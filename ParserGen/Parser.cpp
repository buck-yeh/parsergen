...
enum
{
    ...
    NID_ExprElement          = TID_UB_+2,	// <ExprElement>
    NID_InterBracketed       = TID_UB_+3,	// <InterBracketed>
    NID_Line                 = TID_UB_+4,	// <Line>
    NID_NewLexIds            = TID_UB_+5,	// <NewLexIds>
    NID_OperatorAssociation  = TID_UB_+6,	// <OperatorAssociation>
    NID_OptionalTemplateArgs = TID_UB_+7,	// <OptionalTemplateArgs>
    NID_PreProcLine          = TID_UB_+8,	// <PreProcLine>
    NID_Production           = TID_UB_+9,	// <Production>
    NID_QualifiedName        = TID_UB_+10,	// <QualifiedName>
    NID_Semantics            = TID_UB_+11,	// <Semantics>
    NID_ShiftBase            = TID_UB_+12,	// <ShiftBase>
    NID_TemplateArg          = TID_UB_+13,	// <TemplateArg>
    NID_TemplateArgList      = TID_UB_+14,	// <TemplateArgList>
...
constinit const struct C_MapGoto {
    ...
    {L2S_Table0,	25,	-13},
    {isKey<ZIP_TOKEN(TID_LEX_Bracketed)>,	68,	80},
...
//  <Line> ::= class <QualifiedName> <OptionalTemplateArgs>
{
    ...
    if (auto targs = bux::tryUnlex<C_TemplateArgs>(_geT_(2)))
        ok = c.setClassName(bux::unlex<C_StringList>(_geT_(1)), *targs, errMsg);
    else
    {
        C_TemplateArgs t;
        ...
        ok = c.setClassName(bux::unlex<C_StringList>(_geT_(1)), t, errMsg);
    }

    if (!ok)
        _paRSeR_.onError(_geT_(0), errMsg);
}
...
void _reduce_32(bux::LR1::C_Parser &_paRSeR_, const F_GetProduced &_geT_, C_RetLval _reT_)
...
void _reduce_35(bux::LR1::C_Parser &, const F_GetProduced &_geT_, C_RetLval _reT_)
...
void _reduce_36(bux::LR1::C_Parser &, const F_GetProduced &_geT_, C_RetLval _reT_)
//  <ExprElement> ::= $Id
//  <ExprElement> ::= <@keyword>
{
...
void _reduce_47(bux::LR1::C_Parser &, const F_GetProduced &, C_RetLval _reT_)
//  <@keyword> ::= prec
{
    _reT_ = createLex<std::string>("prec");
}

void _reduce_48(bux::LR1::C_Parser &, const F_GetProduced &, C_RetLval _reT_)
//  <@keyword> ::= right
{
...
{_reduce_13,	ZIP_TOKEN(NID_OperatorAssociation), 1},	// 19: <OperatorAssociation> ::= <AssocType>
{_reduce_14,	ZIP_TOKEN(NID_OperatorAssociation), 2},	// 20: <OperatorAssociation> ::= <OperatorAssociation> <ExprElement>
{_reduce_15,	ZIP_TOKEN(NID_AssocType),   1},	// 21: <AssocType> ::= left
{_reduce_16,	ZIP_TOKEN(NID_AssocType),   1},	// 22: <AssocType> ::= right
{_reduce_17,	ZIP_TOKEN(NID_AssocType),   1},	// 23: <AssocType> ::= prec
{nullptr,    	ZIP_TOKEN(NID_Semantics),   0},	// 24: <Semantics> ::=
{_reduce_18,	ZIP_TOKEN(NID_Semantics),   3},	// 25: <Semantics> ::= <Semantics> $Bracketed <InterBracketed>
{nullptr,    	ZIP_TOKEN(NID_InterBracketed), 0},	// 26: <InterBracketed> ::=
{_reduce_19,	ZIP_TOKEN(NID_InterBracketed), 1},	// 27: <InterBracketed> ::= $LexSymbol
{_reduce_20,	ZIP_TOKEN(NID_InterBracketed), 2},	// 28: <InterBracketed> ::= % $Id
{nullptr,    	ZIP_TOKEN(NID_ShiftBase),   0},	// 29: <ShiftBase> ::=
{_reduce_13,	ZIP_TOKEN(NID_ShiftBase),   1},	// 30: <ShiftBase> ::= $Num
{_reduce_21,	ZIP_TOKEN(NID_ShiftBase),   2},	// 31: <ShiftBase> ::= + $Num
{_reduce_22,	ZIP_TOKEN(NID_ShiftBase),   2},	// 32: <ShiftBase> ::= - $Num
{_reduce_23,	ZIP_TOKEN(NID_Production),  2},	// 33: <Production> ::= $Nonterminal ::=
{_reduce_24,	ZIP_TOKEN(NID_Production),  2},	// 34: <Production> ::= <Production> $Nonterminal
{_reduce_25,	ZIP_TOKEN(NID_Production),  2},	// 35: <Production> ::= <Production> <ExprElement>
{_reduce_26,	ZIP_TOKEN(NID_QualifiedName), 1},	// 36: <QualifiedName> ::= $Id
{_reduce_27,	ZIP_TOKEN(NID_QualifiedName), 3},	// 37: <QualifiedName> ::= <QualifiedName> :: $Id
{_reduce_28,	ZIP_TOKEN(NID_TemplateArgList), 1},	// 38: <TemplateArgList> ::= <TemplateArg>
{_reduce_29,	ZIP_TOKEN(NID_TemplateArgList), 3},	// 39: <TemplateArgList> ::= <TemplateArgList> , <TemplateArg>
{_reduce_26,	ZIP_TOKEN(NID_TemplateArg), 1},	// 40: <TemplateArg> ::= $Id
{_reduce_30,	ZIP_TOKEN(NID_TemplateArg), 2},	// 41: <TemplateArg> ::= <TemplateArg> $Id
{_reduce_31,	ZIP_TOKEN(NID_TemplateArg), 2},	// 42: <TemplateArg> ::= <TemplateArg> *
{_reduce_32,	ZIP_TOKEN(NID_ExprElement), 1},	// 43: <ExprElement> ::= $LexSymbol
{_reduce_33,	ZIP_TOKEN(NID_ExprElement), 1},	// 44: <ExprElement> ::= $String
{_reduce_13,	ZIP_TOKEN(NID_ExprElement), 1},	// 45: <ExprElement> ::= $Num
{_reduce_34,	ZIP_TOKEN(NID_ExprElement), 1},	// 46: <ExprElement> ::= $Key
{_reduce_35,	ZIP_TOKEN(NID_ExprElement), 1},	// 47: <ExprElement> ::= $Operator
{_reduce_35,	ZIP_TOKEN(NID_ExprElement), 1},	// 48: <ExprElement> ::= <@operator>
{_reduce_36,	ZIP_TOKEN(NID_ExprElement), 1},	// 49: <ExprElement> ::= $Id
{_reduce_36,	ZIP_TOKEN(NID_ExprElement), 1},	// 50: <ExprElement> ::= <@keyword>
{nullptr,    	ZIP_TOKEN(bux::ROOT_NID),   1},	// 51: <@> ::= <All>
{_reduce_37,	ZIP_TOKEN(NIDa_operator),   1},	// 52: <@operator> ::= ::
{_reduce_38,	ZIP_TOKEN(NIDa_operator),   1},	// 53: <@operator> ::= ::=
{_reduce_39,	ZIP_TOKEN(NIDa_keyword),    1},	// 54: <@keyword> ::= class
{_reduce_40,	ZIP_TOKEN(NIDa_keyword),    1},	// 55: <@keyword> ::= else
{_reduce_41,	ZIP_TOKEN(NIDa_keyword),    1},	// 56: <@keyword> ::= endif
{_reduce_42,	ZIP_TOKEN(NIDa_keyword),    1},	// 57: <@keyword> ::= ifdef
{_reduce_43,	ZIP_TOKEN(NIDa_keyword),    1},	// 58: <@keyword> ::= ifndef
{_reduce_44,	ZIP_TOKEN(NIDa_keyword),    1},	// 59: <@keyword> ::= include
{_reduce_45,	ZIP_TOKEN(NIDa_keyword),    1},	// 60: <@keyword> ::= left
{_reduce_46,	ZIP_TOKEN(NIDa_keyword),    1},	// 61: <@keyword> ::= lexid
{_reduce_47,	ZIP_TOKEN(NIDa_keyword),    1},	// 62: <@keyword> ::= prec
{_reduce_48,	ZIP_TOKEN(NIDa_keyword),    1} 	// 63: <@keyword> ::= right
};
...
const I_ParserPolicy &C_BNFParser::policy()
{
    return g_policy;
}
