        ...
        grammarPath))
{
    ...
    for (auto &i: actionMap)
    {
        ...
        {
            std::print("Unexpected action-less state {}\n", id);
            m_readySoFar = false;
            return;
        }
        for (auto &j: value)
        ...
        for (auto &j: i.first)
        {
            ...
            if (actions.size() > 1)
            {
                if (!m_Parsed.getOption("SUPPRESS_GLR_CONFLICTS"))
                {
                    ...
                    std::print("GLR needed for lex {} conflics on: {}\n", m_lex2str.find(id)->second, names);
                }
                m_needGLR = true;
            }
        }
...
bool FC_Output::operator()(const char *outputPath, const char *tokenPath, int flags) const
{
    ...
    else
    {
        ...
        if (!m_Parsed.idSet())
        {
            std::print("Yet to set id values of m_Parsed.lex2ID() & m_Parsed.geneTokens()\n");
            return false;
        }
    ...
    if (!m_Parsed.nonterminals().empty())
    {
        ...
        for (auto &i: m_Parsed.nonterminals())
        {
            ...
            std::print(out, "    {} = TID_UB_+{},", s, i.second-tid_ub);
            std::string name;
    ...
    if (!m_alphaPrimes.empty())
    {
        ...
        m_alphaPrimes.visit([&](const C_Alphabet &belt, size_t ind){
            ...
            std::print(out, "{}[{}] = ", alphaPrime(ind), belt.size());
            bool first = true;
            for (auto j: belt)
            {
                out <<(first? '{': ',') <<"\n"
    ...
    for (auto &i: m_iatbl2out)
        if (i.second.m_iaTableInd >= 0)
        {
            if (first)
            {
                std::print(out, "constinit const bux::C_KVPair<{},{}>\n", m_InputType, m_ActionType);
                first = false;
            }
            else
                out <<",\n";
    ...
    {
        std::print(out, "constinit const bux::U_K2V<{},{}> mapActionPtr[{}] =",
    ...
    for (size_t i = 0; const auto r = m_Parsed.getReduction(i); ++i)
    {
        ...
        std::print(out, "void _reduce_{}(bux::{}::C_Parser &", i, nsLR);
        if (ret.second & (1<<0))
            out <<ARG_NAME_PARSER;
    ...
    for (auto &i: m_Parsed.productions())
    {
        ...
        restOfLine = std::format("\t// {}: {}", prodNum, i.str());
        first = false;
    }
    if (acceptID < 0)
...
std::string FC_Output::outputFindKey(const C_Alphabet &alphabet)
{
    ...
    {
        const auto funcName = std::format("findPrime{}_{}_{}", ret->first, ret->second, alphabet.size());
        if (m_findPrimes.find(funcName) == m_findPrimes.end())
            // Not yet defined
        {
            auto out = "return findKey(" + alphaPrime(ret->first);
            if (ret->second)
                out += std::format("+{}", ret->second);

            m_findPrimes[funcName] = out += std::format(",{},key);", alphabet.size());
        }
        return funcName;
    }
    std::string s;
    ...
    for (size_t i =0; (i = src.find('$',i)) != std::string::npos;)
    {
            ...
            // Do replace
        {
            ...
            if (const size_t n = std::strtoul(name.c_str(), 0, 10))
            {
                ...
                {
                    const auto get = std::format(ARG_NAME_GETLEX "({})", n-1);
                    --i;
