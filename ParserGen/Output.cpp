...
FC_Output::FC_Output        (
    ...
    m_Parsed(parsed),
    m_Banner(
        "// This file is fully generated by running " PROGNAME "\n"
        "// with grammer definition file \"" + grammarPath + "\"\n")
{
...
bool FC_Output::operator()(const char *outputPath, const char *tokenPath) const
{
    ...
    auto className = m_Parsed.className();
    if (className.empty())
    {
        className = "C_";
        for (auto c: base)
            className += isascii(c) && isalnum(c)? c: '_';
    }
    std::ofstream   out;
    C_Strings       tokenStrs;
    size_t          maxWidth;
    ...
    fmt::print(out,
        ...
        "public:\n"
        "\n", className, nsLR);
    const auto context = m_Parsed.contextType();
    if (!context.empty())
        // Has parser context
        fmt::print(out,
            "    // Data\n"
            "    {:23} m_context;\n"
            // s.t aslong as context.size() <= 23, m_context will start at column 28
            "\n", context);
    ...
    if (context.empty())
        ...
        // Have a variadic template ctor in convenience of arbitrary context ctor prototypes
        fmt::print(out,
            "    template<class...T_Args>\n"
            "    {0}(T_Args&&...args):\n"
            "        bux::{1}::C_Parser{{{0}::policy()}}, m_context{{std::forward<T_Args>(args)...}}{2}\n"
            "private:\n"
            "\n"
            "    // Statics\n"
            "    static const bux::{1}::I_ParserPolicy &policy();\n",
            className, nsLR, !m_needGLR?
            " {}\n":
            "\n"
            "    {\n"
            "        bux::to_voidptr(m_context);\n"
            "    }\n");
    ...
    const auto parserArgType = m_needGLR? std::string("void *"): std::string("bux::")+nsLR+"::C_Parser &";
    const char *const parserArgName = m_needGLR? "userData": ARG_NAME_PARSER;
    out <<"    void getReduceInfo(size_t id, C_ReduceInfo &info) const override;\n"
          "    void onError(" <<parserArgType <<parserArgName <<", const bux::C_SourcePos &pos, const std::string &message) const override;\n"
          "}   g_policy;\n"
          "\n"
        <<actionRet <<" C_ParserPolicy::action(bux::T_StateID state, bux::T_LexID input) const\n"
          ...
          "\n";
    if (m_needGLR)
        out <<"    auto t = index2values";
    else
        out <<"    return index2value";

    out <<'<' <<m_InputType <<',' <<m_ActionType <<',' <<m_iaTblSzType <<",C_ActionTraits>(mapActionPtr[state], mapActionSizeEx[state], input);\n";
    if (m_needGLR)
    ...
    if (context.empty())
        // Have no context
        out <<className <<"::" <<className <<"(): bux::" <<nsLR <<"::C_Parser(g_policy)\n"
              "{\n"
              "}\n";
    else
        // Have a static policy method
        out <<"const I_ParserPolicy &" <<className <<"::policy()\n"
              "{\n"
              "    return g_policy;\n"
              "}\n";
...
void FC_Output::addParserMap(C_RenderReduction &rr, const std::string &className, size_t argInd) const
{
    ...
    if (m_needGLR)
    {
        const auto context = m_Parsed.contextType();
        if (!context.empty())
            rr.addMap("c", "voidptr_to<"+context+">(" ARG_NAME_PARSER ".userData())", 1<<argInd);
    }
    else
    {
...
void FC_Output::writeUserSection(std::ostream &out, const char *optionKey) const
{
    if (const auto found = m_Parsed.getOption(optionKey))
        out <<"\n"
              "// %" <<optionKey <<" expanded BEGIN\n"
            <<ensureNoConcat(*found)
            <<"// %" <<optionKey <<" expanded END\n";
    else if (m_Parsed.getOption("SHOW_UNDEFINED"))
        out <<"// %" <<optionKey <<" undefined (expanded here otherwise)\n";
}
    ...
    for (size_t i =0; (i = src.find('$',i)) != std::string::npos;)
    {
            ...
            // Do replace
        {
            ...
            if (const size_t n =std::strtoul(name.c_str(), 0, 10))
            {
                ...
                {
                    char get[20] = ARG_NAME_GETLEX "(";
                    sprintf(strchr(get,0), "%d", int(n-1));
                    strcat(get, ")");
                    --i;
