...
std::string banner()
{
    return fmt::format(FMT_STRING("// This file is fully generated by running " PROGNAME " v{}.{}.{}\n"),
        VERSION_MAJOR,
        VERSION_MINOR,
        VERSION_RELEASE);
}
...
{
    fmt::print(FMT_STRING("About to parse '{}' ...\n"), filename);
...
void C_Output::writeCpp(std::ostream &out, const std::string &base) const
{
    ...
    m_DFA.eachFinalState([&](int state, const C_WeightedStrList &action) {
        ...
        if (action.m_1stFit)
            ...
            fmt::print(FMT_STRING("Warning: Non-first-fit action has isFinal: {}\n"), isFinal);
    });
...
const C_WeightedStrList &FC_SolveConflict::operator()(int state, const C_Actions &actions)
{
    ...
    fmt::print(FMT_STRING("Action conflicts at state {}:\n"), state);
    for (const auto &i: actions)
        fmt::print(FMT_STRING("'{}' "), expand(i.m_Expr,m_context));
    fmt::print("\n");
    if (ret == actions.end())
        ret =actions.begin();
    return *ret;
